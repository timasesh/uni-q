...
/** Статические ответы на типовые вопросы о работе сайта uni-q.
 *  Используется вместо LLM — гарантирует точный ответ без галлюцинаций. */
const SITE_STATIC_QA: { pattern: RegExp; answer: string }[] = [
  {
    pattern: /занят.*очередь|встат.*очередь|как встать|как занять|получить.*талон|взять.*талон|записат.*очередь|зарегистр.*очередь|попасть.*очередь|войти.*очередь|как записаться в очередь/iu,
    answer: `Чтобы встать в очередь на сайте uni-q:
1. Нажмите **«Продолжить без входа»** на главной странице.
2. Заполните форму: имя, фамилия, школа, специальность, курс, языковое отделение, форма обучения.
3. Нажмите **«Получить талон»** — вам присвоится порядковый номер в очереди.
4. Ожидайте. Когда подойдёт ваша очередь, прозвучит звуковой сигнал и на экране появится номер окна (**A, B, C или D**).
5. Подойдите к указанному окну — **главный корпус, 2-й этаж, зона у лифтов**.`,
  },
  {
    pattern: /бронир|слот|временн.*запись|выбрать время|удобное время|забронировать/iu,
    answer: `Чтобы забронировать удобное время:
1. Зайдите в **Панель студента** (/student).
2. Нажмите кнопку **«Забронировать время»**.
3. Выберите удобный временной слот на сегодня.
Студент с бронью получает приоритет ближе к выбранному времени. Отменить бронь можно той же кнопкой.`,
  },
  {
    pattern: /что.*значит.*вызван|статус.*вызван|вызван.*статус|меня вызвали/iu,
    answer: `Статус **«ВЫЗВАН»** означает, что ваша очередь подошла. Прозвучит звуковой сигнал, на экране появится номер окна (A, B, C или D). Немедленно подойдите к указанному окну — **главный корпус, 2-й этаж**.`,
  },
  {
    pattern: /что.*значит.*пропущен|статус.*пропущен|пропустил.*очередь/iu,
    answer: `Статус **«ПРОПУЩЕН»** означает, что вас вызвали, но вы не подошли к окну. Вы можете встать в очередь заново, нажав **«Встать в очередь снова»** в Панели студента.`,
  },
  {
    pattern: /что.*значит.*завершено|статус.*завершено|приём.*завершён/iu,
    answer: `Статус **«ЗАВЕРШЕНО»** означает, что ваш приём у менеджера завершён. На экране появится комментарий менеджера. После этого вы можете оставить оценку (1–5 звёзд) и необязательный отзыв.`,
  },
  {
    pattern: /отменить.*очередь|покинуть.*очередь|выйти.*очередь|как отменить|убрать.*талон/iu,
    answer: `Чтобы отменить очередь, нажмите кнопку **«Покинуть очередь»** или **«Отменить»** в Панели студента. После отмены вы можете встать в очередь заново.`,
  },
  {
    pattern: /схем.*офис|офис.*схем|схема.*окн|где.*приёмная|где.*окна|где.*a.*b.*c.*d|план.*офис/iu,
    answer: `Схема офиса доступна по кнопке **«Схема офиса»** в Панели студента. Приёмная находится: **главный корпус, 2-й этаж, зона у лифтов**. Окна A–D — у входа в зону приёма.`,
  },
  {
    pattern: /оставить.*отзыв|оценить.*визит|оценк.*прием|как оценить|поставить.*звезд/iu,
    answer: `После завершения приёма (статус «ЗАВЕРШЕНО») автоматически откроется форма оценки. Выберите **1–5 звёзд** и при желании оставьте комментарий. Перед оценкой отобразится комментарий менеджера.`,
  },
  {
    pattern: /язык.*интерфейс|поменять.*язык|переключ.*язык|сменить.*язык|выбрать.*язык/iu,
    answer: `Язык интерфейса переключается в **верхней части страницы** — кнопки **RUS / ENG / KAZ**. Поддерживаются три языка: русский, английский, казахский.`,
  },
  {
    pattern: /что такое uni.?q|как работает uni.?q|что это за сайт|что за система|как работает система/iu,
    answer: `**uni-q** — это электронная очередь Студенческого сервисного центра университета. Вы получаете номер онлайн, отслеживаете очередь в реальном времени и приходите к окну только когда вас вызовут. Приёмная: **главный корпус, 2-й этаж, зона у лифтов**.`,
  },
];

/** Ищет статический ответ для site-вопросов. Возвращает ответ или null если не найден. */
function findStaticSiteAnswer(text: string): string | null {
  for (const entry of SITE_STATIC_QA) {
...
app.post("/api/student/chat", async (req, res) => {
  ...
  // --- STATIC SITE ANSWER PATH ---
  // Для типовых вопросов о сайте (встать в очередь, статусы, бронирование и т.д.)
  // возвращаем заранее написанный точный ответ — без LLM, без галлюцинаций.
  if (isSiteQuestion) {
    const staticAnswer = findStaticSiteAnswer(lastUserQuestion);
    if (staticAnswer) {
      return res.json({
        reply: staticAnswer,
        source: "static_site",
        kbQuestionNorm: null,
        ...(debugRequested ? { debug: { reason: "static_site", intentNow } } : {}),
      });
    }
  }
  ...
  // Intent compatibility: if user intent is known (not "other"), the KB entry intent must match
  // or be "other" — prevents returning e.g. a payment answer for a registration question.
  const bestKbEntryIntent = bestKb ? detectIntent(`${bestKb.entry.question} ${bestKb.entry.answer}`) : "other";
  const intentCompatible =
    intentNow === "other" ||
    bestKbEntryIntent === "other" ||
    bestKbEntryIntent === intentNow;
  const canReplyDirectKb =
    !isSiteQuestion &&                                              // site questions → LLM with SITE_GUIDE
    !!bestKb &&
    intentCompatible &&                                             // block wrong-intent KB answers
